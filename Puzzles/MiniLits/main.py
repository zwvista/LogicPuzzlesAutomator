...
def _get_combined_pixel_colors(large_img: np.ndarray, line_results, column_results, offset_x=10, offset_y=10):
    """
...
def _compress_colors_to_codes(color_matrix):
    """
    将二维颜色数组中的颜色替换为数字代码。
...
def _create_grid_string(coded_matrix):
    """
...
def _get_level_str_from_image(large_img: np.ndarray) -> str:
    ...
    grid_string = _create_grid_string(coded_matrix)
    return grid_string
