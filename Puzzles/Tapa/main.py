...
def _recognize_text(large_img: np.ndarray, x: int, y: int, w: int, h: int) -> str | None:
    ...
    output = reader.readtext(roi_large, allowlist='0123456789?', detail=0)
    if output and len(output) == 1 and output[0] == '22':
        output = reader.readtext(roi, allowlist='0123456789?', detail=0)
...
def _get_level_str_from_image(large_img: np.ndarray) -> str:
    ...
    matrix = _recognize_digits(large_img, processed_horizontal_lines, processed_vertical_lines)
