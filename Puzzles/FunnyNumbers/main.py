...
class _Analyzer(PuzzleAnalyzer):
    ...
    def __init__(self: Self):
        super().__init__(
            400,
            [(1, 4), (11, 5), (31, 6), (61, 7), (121, 8), (181, 9), (251, 10)]
        )
    ...
    def format_matrix_with_walls2(
        ...
        def adjust_sum(str) -> str:
            str2 = "" if str == "1" else "22" if str == "2" else str
            return str2.rjust(2)
        ...
        for r in range(rows + 1):
            ...
            lines.append(''.join(line) + adjust_sum(digits[cols]) + '`')
        digits = matrix[rows]
        line = []
        for c in range(cols):
            line.append(adjust_sum(digits[c]))
        lines.append(''.join(line) + '   `')
        result = '\n'.join(lines)
        return result
