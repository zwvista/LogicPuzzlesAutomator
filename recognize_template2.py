...
for i, coord in enumerate(list_coords):
    ...
    # 验证SHOP
    print("-> 验证SHOP：")
    found_shop = check_template_in_region_grayscale(
        large_image_path=LARGE_IMAGE_PATH,
        template_path=SHOP_PATH,
    ...
    # 验证GAS
    print("-> 验证GAS：")
    found_gas = check_template_in_region_grayscale(
        large_image_path=LARGE_IMAGE_PATH,
        template_path=GAS_PATH,
        top_left_coord=coord,
        size=size,
        max_diff=MAX_DIFFERENCE
    )

    print(f"区域 {coord} 结果: SHOP={found_shop}, GAS={found_gas}")
